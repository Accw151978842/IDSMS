...
namespace IDSMS.Forms
{
    public class CustomerForm : Form
    {
        ...
        private TextBox txtSrch, txtID, txtName, txtContact, txtEmail, txtAddress, txtCreditLimit;
        ...
        public CustomerForm()
        {
            ...
            txtCreditLimit = new TextBox { Size = new System.Drawing.Size(fw, 23) }; AddRow(px, ref py, lw, fw, "Credit Limit:", txtCreditLimit);
            ...
            try
            {
                using (MySqlConnection cn = DBConnection.GetConnection())
                {
                    ...
                    string sql = string.IsNullOrEmpty(s)
                        ? "SELECT customerID,name,contact,email FROM customer ORDER BY name"
                        : "SELECT customerID,name,contact,email FROM customer WHERE name LIKE @s OR customerID LIKE @s";
                    using (MySqlCommand cmd = new MySqlCommand(sql, cn))
                    {
                        if (!string.IsNullOrEmpty(s)) cmd.Parameters.AddWithValue("@s", "%" + s + "%");
                        DataTable dt = new DataTable(); new MySqlDataAdapter(cmd).Fill(dt); dgv.DataSource = dt;
                    }
                }
            }
        ...
        private void Sel(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    cn.Open();
                    using (MySqlCommand cmd = new MySqlCommand("SELECT * FROM customer WHERE customerID=@id", cn))
                    {
                        cmd.Parameters.AddWithValue("@id", selID);
                        using (MySqlDataReader r = cmd.ExecuteReader())
                        {
                            if (r.Read()) { txtID.Text = r["customerID"].ToString(); txtName.Text = r["name"].ToString(); txtContact.Text = r["contact"].ToString(); txtEmail.Text = r["email"].ToString(); txtAddress.Text = r["address"].ToString(); txtCreditLimit.Text = r["creditlimit"].ToString(); }
                        }
                    }
                }
            }
        ...
        private void New_()
        {
            selID = "CUST-" + DateTime.Now.ToString("yyyyMMddHHmmss"); txtID.Text = selID;
            txtName.Text = txtContact.Text = txtEmail.Text = txtAddress.Text = ""; txtCreditLimit.Text = "0";
        }
        ...
        private void Save_()
        {
            ...
            try
            {
                using (MySqlConnection cn = DBConnection.GetConnection())
                {
                    cn.Open();
                    bool ex;
                    using (MySqlCommand chk = new MySqlCommand("SELECT COUNT(*) FROM customer WHERE customerID=@id", cn))
                    {
                        chk.Parameters.AddWithValue("@id", txtID.Text); ex = (long)chk.ExecuteScalar() > 0;
                    }
                    string sql = ex
                        ? "UPDATE customer SET name=@n,contact=@c,email=@e,address=@a,creditlimit=@cl WHERE customerID=@id"
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(sql, cn))
                    {
                        ...
                        decimal cl = 0; decimal.TryParse(txtCreditLimit.Text, out cl); cmd.Parameters.AddWithValue("@cl", cl);
                        cmd.ExecuteNonQuery();
                    }
        ...
        private void Del_()
        {
            ...
            try
            {
                ...
                {
                    cn.Open();
                    using (MySqlCommand cmd = new MySqlCommand("DELETE FROM customer WHERE customerID=@id", cn))
                    {
