...
namespace IDSMS.Forms
{
    public class ShipmentTrackingForm : Form
    {
        ...
        private TextBox txtSrch,txtID,txt_orderID,txt_dnID,txt_driver;
        private ComboBox cbo_status;
        private DateTimePicker dtp_dispatchdate;
        ...
        public ShipmentTrackingForm(string filterID)
        {
            ...
            Row("Shipment ID:",txtID);
            txt_orderID=new TextBox{Size=new System.Drawing.Size(fw,23)};Row("Order ID:",txt_orderID);
            txt_dnID=new TextBox{Size=new System.Drawing.Size(fw,23)};Row("DN ID:",txt_dnID);
            ...
            cbo_status.Items.AddRange(new object[]{"Preparing","InTransit","Delivered","Failed"});Row("Status:",cbo_status);
        ...
        private void Sel(object? s,EventArgs e){
            ...
            try{using var cn=DBConnection.GetConnection();cn.Open();
                using var cmd=new MySqlCommand("SELECT * FROM shipments WHERE shipmentID=@id",cn);
                cmd.Parameters.AddWithValue("@id",selID);
                using var r=cmd.ExecuteReader();
                if(r.Read()){txtID.Text=r["shipmentID"]?.ToString()??"";
                    txt_orderID.Text=r["orderID"]?.ToString()??"";
                    txt_dnID.Text=r["dnID"]?.ToString()??"";
        ...
        private void New_(){selID="SHP-"+DateTime.Now.ToString("yyyyMMddHHmmss");txtID.Text=selID;
            txt_orderID.Text=txt_dnID.Text=txt_driver.Text="";
            dtp_dispatchdate.Value=DateTime.Today;cbo_status.Text="Preparing";}
        private void Save_(){
            if(string.IsNullOrEmpty(txtID.Text)){MessageBox.Show("Click New first.");return;}
            try{using var cn=DBConnection.GetConnection();cn.Open();
                bool ex;using(var chk=new MySqlCommand("SELECT COUNT(*) FROM shipments WHERE shipmentID=@id",cn)){chk.Parameters.AddWithValue("@id",txtID.Text);ex=(long)chk.ExecuteScalar()>0;}
                ...
                cmd.Parameters.AddWithValue("@dn",txt_dnID.Text);cmd.Parameters.AddWithValue("@dd",dtp_dispatchdate.Value.Date);
                cmd.Parameters.AddWithValue("@dr",txt_driver.Text);cmd.Parameters.AddWithValue("@ss",cbo_status.Text);
                cmd.ExecuteNonQuery();
        ...
        private void Del_(){
            ...
            try{using var cn=DBConnection.GetConnection();cn.Open();
                using var cmd=new MySqlCommand("DELETE FROM shipments WHERE shipmentID=@id",cn);
