...
{
    public class ShipmentTrackingForm : Form
    {
        private DataGridView dgv;
        private TextBox txtSrch, txtID, txtOrderID, txtDnID, txtDriver;
        private ComboBox cboStatus;
        private DateTimePicker dtpDispatch;
        private string selID = "";

        public ShipmentTrackingForm() : this("") { }
        public ShipmentTrackingForm(string filterID)
        {
            Text = "Shipment Tracking"; Size = new System.Drawing.Size(1000, 540); StartPosition = FormStartPosition.CenterParent;
            ...
            Controls.Add(new Label { Text = "Shipment Tracking", Location = new System.Drawing.Point(px, py - 22), Font = new System.Drawing.Font("Segoe UI", 9, System.Drawing.FontStyle.Bold) });
            txtID = new TextBox { Size = new System.Drawing.Size(fw, 23), ReadOnly = true };
            AddRow(px, ref py, lw, fw, "Shipment ID:", txtID);
            txtOrderID = new TextBox { Size = new System.Drawing.Size(fw, 23) }; AddRow(px, ref py, lw, fw, "Order ID:", txtOrderID);
            txtDnID = new TextBox { Size = new System.Drawing.Size(fw, 23) }; AddRow(px, ref py, lw, fw, "DN ID:", txtDnID);
            ...
            cboStatus.Items.AddRange(new object[] { "Preparing", "InTransit", "Delivered", "Failed" }); AddRow(px, ref py, lw, fw, "Status:", cboStatus);
        ...
        private void Reload(string s)
        {
            try
            {
                using (MySqlConnection cn = DBConnection.GetConnection()) { cn.Open();
                    string sql = string.IsNullOrEmpty(s) ? "SELECT * FROM shipments ORDER BY shipmentID DESC LIMIT 300" : "SELECT * FROM shipments WHERE shipmentID LIKE @s OR orderID LIKE @s LIMIT 300";
        ...
        private void Sel(object sender, EventArgs e)
        {
            ...
            selID = row["shipmentID"].ToString();
            try
            {
                using (MySqlConnection cn = DBConnection.GetConnection()) { cn.Open();
                    using (MySqlCommand cmd = new MySqlCommand("SELECT * FROM shipments WHERE shipmentID=@id", cn)) { cmd.Parameters.AddWithValue("@id", selID);
                        using (MySqlDataReader r = cmd.ExecuteReader()) { if (r.Read()) { txtID.Text = r["shipmentID"].ToString(); txtOrderID.Text = r["orderID"].ToString(); txtDnID.Text = r["dnID"].ToString(); if (r["dispatchdate"] != DBNull.Value) dtpDispatch.Value = Convert.ToDateTime(r["dispatchdate"]); txtDriver.Text = r["driver"].ToString(); cboStatus.Text = r["status"].ToString(); } } } }
            }
            catch { }
        }

        private void New_() { selID = "SHP-" + DateTime.Now.ToString("yyyyMMddHHmmss"); txtID.Text = selID; txtOrderID.Text = txtDnID.Text = txtDriver.Text = ""; dtpDispatch.Value = DateTime.Today; cboStatus.Text = "Preparing"; }
        ...
        private void Save_()
        {
            ...
            {
                using (MySqlConnection cn = DBConnection.GetConnection()) { cn.Open();
                    bool ex; using (MySqlCommand chk = new MySqlCommand("SELECT COUNT(*) FROM shipments WHERE shipmentID=@id", cn)) { chk.Parameters.AddWithValue("@id", txtID.Text); ex = (long)chk.ExecuteScalar() > 0; }
                    string sql = ex ? "UPDATE shipments SET orderID=@o,dnID=@dn,dispatchdate=@dd,driver=@dr,status=@ss WHERE shipmentID=@id"
                                   : "INSERT INTO shipments(shipmentID,orderID,dnID,dispatchdate,driver,status)VALUES(@id,@o,@dn,@dd,@dr,@ss)";
                    using (MySqlCommand cmd = new MySqlCommand(sql, cn)) { cmd.Parameters.AddWithValue("@id", txtID.Text); cmd.Parameters.AddWithValue("@o", txtOrderID.Text); cmd.Parameters.AddWithValue("@dn", txtDnID.Text); cmd.Parameters.AddWithValue("@dd", dtpDispatch.Value.Date); cmd.Parameters.AddWithValue("@dr", txtDriver.Text); cmd.Parameters.AddWithValue("@ss", cboStatus.Text); cmd.ExecuteNonQuery(); }
        ...
        private void Del_()
        {
            ...
            try { using (MySqlConnection cn = DBConnection.GetConnection()) { cn.Open(); using (MySqlCommand cmd = new MySqlCommand("DELETE FROM shipments WHERE shipmentID=@id", cn)) { cmd.Parameters.AddWithValue("@id", selID); cmd.ExecuteNonQuery(); } selID = ""; Reload(""); } }
