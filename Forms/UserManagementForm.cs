...
namespace IDSMS.Forms
{
    public class UserManagementForm : Form
    {
        ...
        public UserManagementForm()
        {
            ...
            txtID=new TextBox{Size=new System.Drawing.Size(fw,23),ReadOnly=true};Row("User ID:",txtID);
            txtName=new TextBox{Size=new System.Drawing.Size(fw,23)};Row("Name *:",txtName);
            txtEmail=new TextBox{Size=new System.Drawing.Size(fw,23)};Row("Email:",txtEmail);
            cboRole=new ComboBox{Size=new System.Drawing.Size(fw,23),DropDownStyle=ComboBoxStyle.DropDownList};
            cboRole.Items.AddRange(new object[]{"Admin","Sales","Production","Inventory","Logistics","Finance"});Row("Role *:",cboRole);
            txtPwd=new TextBox{Size=new System.Drawing.Size(fw,23),UseSystemPasswordChar=true};Row("Password:",txtPwd);
        ...
        private void Sel(object? s,EventArgs e){
            ...
            txtID.Text=selID;txtName.Text=row["empname"]?.ToString()??"";
            cboRole.Text=row["role"]?.ToString()??"";
            txtEmail.Text=row["email"]?.ToString()??"";
            txtPwd.Text="";}
        private void New_(){selID="USR"+DateTime.Now.ToString("yyMMddHHmmss");txtID.Text=selID;
            txtName.Text=txtEmail.Text=txtPwd.Text="";cboRole.SelectedIndex=-1;}
        private void Save_(){
            ...
            try{using var cn=DBConnection.GetConnection();cn.Open();
                bool ex;using(var chk=new MySqlCommand("SELECT COUNT(*) FROM users WHERE userID=@id",cn)){chk.Parameters.AddWithValue("@id",txtID.Text);ex=(long)chk.ExecuteScalar()>0;}
                ...
                else{if(!chgPwd){MessageBox.Show("Password required for new user.");return;}
                    sql="INSERT INTO users(userID,empname,role,email,password)VALUES(@id,@n,@r,@e,@p)";}
                using var cmd=new MySqlCommand(sql,cn);
                cmd.Parameters.AddWithValue("@id",txtID.Text);cmd.Parameters.AddWithValue("@n",txtName.Text);
                cmd.Parameters.AddWithValue("@r",cboRole.Text);cmd.Parameters.AddWithValue("@e",txtEmail.Text);
                if(chgPwd)cmd.Parameters.AddWithValue("@p",txtPwd.Text);
                cmd.ExecuteNonQuery();Reload();
        ...
        private void Del_(){
            ...
            try{using var cn=DBConnection.GetConnection();cn.Open();
                using var cmd=new MySqlCommand("DELETE FROM users WHERE userID=@id",cn);
